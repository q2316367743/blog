...
export default {
    ...
    data: function() {
        ...
        return {
            ...
            logo_url: '', // logo地址 
            icon_url: '', // icon地址 
            github_url: 'https://gitee.com/qiaoshengda/website', // github地址 
    ...
    methods: {
        ...
        printVesion: function() {
            console.log('欢迎使用个人网站，当前版本：' + this.version + "，更新于：" + this.update_time + "，GitHub地址：" + this.github_url);
        },
        // 初始化window相关配置 
        initWindow: function() {
    ...
    mounted: function() {
        ...
        if (window.sessionStorage.getItem('satoken')) {
            info(res => {
                if (res.success) {
                    this.$store.commit('LOGIN', res.data.item)
                } else {
                    this.sa.error2('获取个人信息错误')
                }
            })
        } else {
            this.openLogin();
