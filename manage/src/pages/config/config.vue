				...
				<el-form ref="form" :model="config" label-width="80px">
					<el-form-item label="背景图片">
						...
						<el-input
							...
							:disabled="!background_input"
							class="config-input"
						></el-input>
						<br />
						<el-checkbox v-model="background_input"
							>外部连接？</el-checkbox
						>
					</el-form-item>
					<el-form-item
						label="音乐"
						v-for="(item, index) in config.music"
						:key="index"
					>
						...
						<el-form-item
							v-if="config.music.length === index + 1"
							style="margin-top: 10px"
						>
							<el-button type="primary" @click="add_music"
								>新增</el-button
							>
						</el-form-item>
					</el-form-item>
	...
	methods: {
		handleBackgroundSuccess(res) {
			this.config.background =
				this.sa.cfg.web_url + "image/" + res.data.item;
		},
		add_music() {
		...
		update() {
			this.sa.put("config", this.config, (res) => {
				if (res.success) {
					this.sa.ok2("修改成功");
